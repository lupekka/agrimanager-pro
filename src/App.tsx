...
// --- COMPONENTE DINASTIA (FIXED PARSE ERROR) ---
...
export default function App() {
  ...
  // FIX BUG DEL CONTEGGIO: Solo capi con nome reale e ID
  ...
  // FIX IMMAGINE 2: Aggiunta funzione reduceProduct
  ...
  const handleSaveBirth = async () => {
    ...
    for (let i = 0; i < newBirth.count; i++) {
      await addDoc(collection(db, 'animals'), {
        ...
        dam: newBirth.idCode,
        notes: `Parto automatico del ${newBirth.birthDate}`,
        ownerId: user?.uid
      });
    }
        ...
        {activeTab === 'inventory' && (
          <div className="space-y-12">
            <div className="bg-white p-10 rounded-[2.5rem] border border-stone-100 shadow-xl">
              <h3 className="text-xs font-black mb-8 text-emerald-900 uppercase tracking-widest flex items-center gap-2"><PlusCircle size={20} className="text-emerald-500" /> Registrazione Capo</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <input placeholder="Identificativo" className="ui-input w-full" value={newAnimal.name} onChange={e => setNewAnimal({...newAnimal, name: e.target.value})} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {capi.map(a => (
                      <div key={a.id} className="bg-white p-8 rounded-[2rem] border border-stone-100 relative group shadow-sm hover:shadow-xl transition-all">
                        ...
                        <p className="text-[10px] text-stone-400 font-black uppercase mb-4 tracking-widest italic">{a.birthDate || 'DATA N/D'}</p>
        ...
        {activeTab === 'finance' && (
          <div className="space-y-12">
                            ...
                            {specTrans.sort((a,b) => b.date.localeCompare(a.date)).map(t => (
                                <div key={t.id} className="flex justify-between items-center p-3 bg-stone-50 rounded-xl group hover:bg-white transition">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className={t.type === 'Entrata' ? 'text-emerald-600 font-black' : 'text-red-500 font-black'}>€{t.amount}</span>
                                        <button onClick={() => deleteDoc(doc(db, 'transactions', t.id))} className="text-stone-300 hover:text-red-500 md:opacity-0 group-hover:opacity-100"><Trash2 size={14}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="bg-white p-10 rounded-[2.5rem] border shadow-xl">
              <h3 className="text-xs font-black uppercase mb-6">Nuovo Movimento</h3>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <button onClick={async () => { if(!newTrans.desc) return; await addDoc(collection(db, 'transactions'), { ...newTrans, date: new Date().toLocaleDateString('it-IT'), ownerId: user.uid }); setNewTrans({desc:'', amount:0, type:'Entrata', species:'Maiali'}); }} className="bg-emerald-600 text-white font-black rounded-2xl py-4 shadow-lg hover:bg-emerald-700 uppercase text-[10px] tracking-widest">Salva</button>
              </div>
            </div>
          </div>
        )}
