...
import { 
  PawPrint, CalendarDays, TrendingUp, Network, Baby, Milk, Trash2, 
    ...
    <div className={`${level > 0 ? 'ml-6 border-l-2 border-emerald-100' : ''} mt-2`}>
      <div className={`flex items-center gap-3 p-4 bg-white rounded-xl shadow-sm border ${level === 0 ? 'border-l-4 border-emerald-600' : 'border-gray-100 ml-2'}`}>
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-gray-400 uppercase font-black tracking-tighter">Gen. {level} • {animal.species}</span>
        </div>
      </div>
...
export default function App() {
  ...
  // --- LOGICA AZIONI (RISOLVE ERRORI FOTO 4 E 5) ---

  const handleSaveBirth = async () => {
    if (!newBirth.idCode || newBirth.count <= 0) return;
    await addDoc(collection(db, 'births'), { animalName: newBirth.idCode, species: newBirth.species, date: new Date().toLocaleDateString(), offspringCount: newBirth.count, birthDate: newBirth.birthDate, ownerId: user?.uid });
    
    // Automazione: Crea N record nell'inventario
    for (let i = 0; i < newBirth.count; i++) {
      await addDoc(collection(db, 'animals'), {
        ...
        dam: newBirth.idCode,
        notes: `Nato il ${newBirth.birthDate} da parto registrato`,
        ownerId: user?.uid
      });
    }
  ...
  // Funzione definita correttamente per l'Immagine 4
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#F8F9FA] text-stone-900 font-sans">
      <style>{`.ui-input { background: white; padding: 0.85rem 1.25rem; border-radius: 1rem; border: 1.5px solid #E9ECEF; outline: none; transition: 0.3s; font-weight: 600; } .ui-input:focus { border-color: #059669; }`}</style>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-40 w-80 bg-white border-r border-stone-100 p-8 flex flex-col transform transition-transform duration-500 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <button onClick={() => signOut(auth)} className="mt-8 flex items-center gap-2 text-red-500 font-black uppercase text-[10px] tracking-widest p-4 hover:bg-red-50 rounded-2xl"><LogOut size={16} /> Esci</button>
      </aside>
      ...
      <main className="flex-1 p-6 md:p-14 h-screen overflow-y-auto scroll-smooth">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
            ...
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-stone-100 text-center">
                <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1">Saldo</p>
          ...
          <div className="space-y-12">
            <div className="bg-white p-10 rounded-[2.5rem] border border-stone-100 shadow-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <select className="ui-input w-full text-xs" value={newAnimal.sire} onChange={e => setNewAnimal({...newAnimal, sire: e.target.value})}><option value="">Padre (Ignoto)</option>{animals.filter(a => a.species === newAnimal.species).map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select>
                <select className="ui-input w-full text-xs" value={newAnimal.dam} onChange={e => setNewAnimal({...newAnimal, dam: e.target.value})}><option value="">Madre (Ignota)</option>{animals.filter(a => a.species === newAnimal.species).map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select>
              </div>
            ...
            {speciesList.map(species => (
              <div key={species} className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <span className="bg-white border px-4 py-1.5 rounded-full text-xs font-black text-emerald-700">{animals.filter(a => a.species === species).length} CAPI</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {animals.filter(a => a.species === species).map(a => (
                    <div key={a.id} className="bg-white p-8 rounded-[2rem] border border-stone-100 relative group shadow-sm hover:shadow-xl transition-all">
                      ...
                      <p className="text-[10px] text-stone-400 font-black uppercase mb-4 flex items-center gap-2"><CalendarDays size={14} /> {a.birthDate || 'DATA N/D'}</p>
                      {a.notes && <div className="mt-4 p-4 bg-emerald-50/30 rounded-2xl text-xs text-stone-600 italic border-l-4 border-emerald-500">{a.notes}</div>}
                      <button onClick={() => deleteDoc(doc(db, 'animals', a.id))} className="absolute top-4 right-4 text-stone-300 hover:text-red-600 md:opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={18}/></button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-10">
            <div className="bg-white p-10 rounded-[2.5rem] border shadow-xl">
              <div className="flex flex-col md:flex-row gap-6 items-end">
                ...
                <div className="w-full md:w-48"><label className="text-[10px] font-black text-stone-400 mb-2 block uppercase">Specie</label><select className="ui-input w-full" value={newBirth.species} onChange={e => setNewBirth({...newBirth, species: e.target.value as Species})}>{speciesList.map(s => <option key={s}>{s}</option>)}</select></div>
                <div className="w-full md:w-24"><label className="text-[10px] font-black text-stone-400 mb-2 block uppercase">Nati</label><input type="number" className="ui-input w-full" value={newBirth.count} onChange={e => setNewBirth({...newBirth, count: parseInt(e.target.value)})} /></div>
                <div className="w-full md:w-48"><label className="text-[10px] font-black text-stone-400 mb-2 block uppercase">Data Nascita</label><input type="date" className="ui-input w-full" value={newBirth.birthDate} onChange={e => setNewBirth({...newBirth, birthDate: e.target.value})} /></div>
            ...
            <div className="bg-white rounded-[2rem] border divide-y overflow-hidden">
              {births.map(b => <div key={b.id} className="p-6 flex justify-between items-center hover:bg-stone-50 transition"><div><p className="font-black text-xl italic">{b.animalName} <span className="text-[10px] px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 ml-4 font-black">{b.species}</span></p><p className="text-xs text-stone-400 font-bold mt-2 tracking-widest">{b.offspringCount} nati il {b.birthDate}</p></div><button onClick={() => deleteDoc(doc(db, 'births', b.id))} className="text-red-300 hover:text-red-500"><Trash2 size={20}/></button></div>)}
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              {products.map(p => (
                <div key={p.id} className="bg-white p-8 rounded-[2rem] border border-stone-100 text-center shadow-sm relative group hover:border-emerald-200 transition-all">
                  <div className="bg-emerald-50 w-16 h-16 rounded-3xl flex items-center justify-center text-emerald-600 mx-auto mb-6"><Package size={32} /></div>
                  ...
                  <div className="flex gap-2 justify-center pt-6 border-t border-stone-50">
                    <button onClick={() => reduceProduct(p.id, 1)} className="p-3 bg-stone-50 text-stone-400 rounded-xl hover:text-red-500"><MinusCircle size={24}/></button>
                    <button onClick={() => deleteDoc(doc(db, 'products', p.id))} className="p-3 bg-stone-50 text-stone-400 rounded-xl hover:text-red-500"><Trash2 size={24}/></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-10">
            <div className="bg-emerald-950 p-12 rounded-[3rem] text-white flex flex-col md:flex-row justify-between items-center shadow-2xl relative overflow-hidden">
                <div className="relative z-10 text-center md:text-left">
                    <p className="text-emerald-300 text-[10px] font-black uppercase tracking-[0.3em] mb-2">Bilancio Totale</p>
                ...
                <div className="grid grid-cols-2 gap-4 relative z-10">
                    <div className="bg-emerald-900/50 p-6 rounded-3xl border border-emerald-800"><p className="text-[9px] font-black uppercase mb-1">Entrate</p><p className="text-2xl font-black">+ €{transactions.filter(t=>t.type==='Entrata').reduce((acc,t)=>acc+t.amount,0).toFixed(0)}</p></div>
                    <div className="bg-red-900/30 p-6 rounded-3xl border border-red-800/50"><p className="text-[9px] font-black uppercase mb-1">Uscite</p><p className="text-2xl font-black">- €{transactions.filter(t=>t.type==='Uscita').reduce((acc,t)=>acc+t.amount,0).toFixed(0)}</p></div>
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-white p-8 rounded-3xl border shadow-sm">
              ...
              <button onClick={async () => { if(!newTrans.desc) return; await addDoc(collection(db, 'transactions'), { ...newTrans, date: new Date().toLocaleDateString('it-IT'), ownerId: user.uid }); setNewTrans({desc:'', amount:0, type:'Entrata', species:'Maiali'}); }} className="bg-emerald-600 text-white font-black rounded-2xl py-4 shadow-lg hover:bg-emerald-700 transition-all text-[10px] uppercase tracking-widest">Salva</button>
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 pb-24">
                <div className="bg-white p-10 rounded-[3rem] border shadow-xl">
                    ...
                    <div className="space-y-3">
                        {tasks.filter(t => !t.done).map(t => (
                            <div key={t.id} className="flex items-center justify-between p-6 bg-stone-50 rounded-[1.5rem] group hover:bg-emerald-50 border border-stone-100">
                                ...
                                <button onClick={() => deleteDoc(doc(db, 'tasks', t.id))} className="text-red-200 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={20}/></button>
                            </div>
                        ))}
                    </div>
                </div>
                <div className="bg-white p-10 rounded-[3rem] border shadow-xl flex flex-col">
                    <div className="flex justify-between items-center mb-8 border-b pb-6">
                        <h3 className="text-xl font-black italic flex items-center gap-3"><History size={24} className="text-stone-400" /> Storico</h3>
