...
import {
  ...
  Camera, Bell, BellRing, Syringe, CalendarClock, AlertCircle
} from 'lucide-react';
...
export default function App() {
  ...
  const [buyerInfo, setBuyerInfo] = useState<{ itemId: string | null; name: string; message: string }>({
    itemId: null,
    name: '',
    message: ''
  });
  ...
  const treatmentTypes: TreatmentType[] = ['Vaccino', 'Vermifugo', 'Visita', 'Cura', 'Altro'];
  ...
  useEffect(() => {
    ...
    // Se non è ancora pronto, aspetta
    const interval = setInterval(checkOneSignal, 1000);
    
    return () => clearInterval(interval);
  }, [oneSignalInitialized, user]);

  // FUNZIONE PER INVIARE NOTIFICA CON ONESIGNAL
  const sendOneSignalNotification = async (title: string, message: string, data?: any) => {
    if (!window.OneSignal || !oneSignalId) {
      console.log('❌ OneSignal non disponibile');
      return;
    }

    try {
      // Mostra notifica in-app se l'app è in primo piano
      ...
      // Per notifiche remote, OneSignal le gestisce automaticamente
      // Possiamo salvare la notifica nel database per tracciamento
      if (user) {
      ...
      console.log('✅ Notifica OneSignal pronta:', title);
    } catch (error) {
      ...
      if (u) {
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            // Se OneSignal è già inizializzato, collega l'utente
            if (window.OneSignal && oneSignalId) {
  ...
  useEffect(() => {
    ...
    if (userRole === 'farmer') {
      ...
      unsubs.push(onSnapshot(q('transactions'), (s) => 
        setTransactions(s.docs.map(d => ({ id: d.id, ...d.data() } as Transaction)))
      ));
      unsubs.push(onSnapshot(q('tasks'), (s) => 
        setTasks(s.docs.map(d => ({ id: d.id, ...d.data() } as Task)))
      ));
      unsubs.push(onSnapshot(q('products'), (s) => 
        setProducts(s.docs.map(d => ({ id: d.id, ...d.data() } as Product)))
      ));
      unsubs.push(onSnapshot(query(collection(db, 'stock_logs'), where("ownerId", "==", user.uid), orderBy('date', 'desc'), limit(15)), (s) => 
        setStockLogs(s.docs.map(d => ({ id: d.id, ...d.data() } as StockLog)))
      ));
    }
  ...
  const checkExpiringTreatments = (animals: Animal[]) => {
    ...
    // Invia notifica UNA VOLTA AL GIORNO
    const lastCheck = localStorage.getItem('lastNotificationCheck');
    const todayStr = today.toDateString();
    
    if (lastCheck !== todayStr && expiring.length > 0) {
      // Invia notifica per il primo trattamento in scadenza
      const first = expiring[0];
      sendOneSignalNotification(
        `📅 Scadenza: ${first.animalName}`,
        `${first.treatment.tipo} in scadenza tra ${first.daysLeft} giorni`,
        { 
          animalName: first.animalName,
          treatmentId: first.treatment.id,
  ...
  const handleAddTreatment = async () => {
    ...
    if (!newTreatment.dataSomministrazione) return alert("Inserisci data di somministrazione");
  ...
  const handlePublishToMarket = async () => {
    ...
    try {
      await addDoc(collection(db, 'market_items'), { 
        ...
        sellerId: user!.uid, 
        sellerName: userName || user!.email?.split('@')[0] || 'Azienda Agricola', 
        contactEmail: user!.email || '', 
        contactPhone: sellPhone, 
        createdAt: new Date().toISOString(),
        description: `${sellingProduct.quantity} ${sellingProduct.unit} di ${sellingProduct.name}`
      });
  ...
  const handlePurchase = async (item: MarketItem) => {
    ...
    try {
      if (userRole === 'farmer') {
        await addDoc(collection(db, 'transactions'), {
          desc: `Acquisto ${item.name} da ${item.sellerName}`,
      ...
      alert(`✅ Acquisto completato! Contatta ${item.sellerName} via ${item.contactPhone ? 'WhatsApp' : 'email'}.`);
    } catch (error) {
  ...
  const getDiagnosisFromSymptoms = (symptoms: string): any => {
        ...
        <div className="bg-white p-6 rounded-3xl shadow-xl w-full max-w-sm border">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <input 
              ...
              required 
            />
            <button type="submit" className="w-full bg-emerald-700 text-white py-3 rounded-xl font-bold uppercase shadow-lg active:scale-95">
              {isRegistering ? "Crea Account" : "Entra"}
            </button>
            <button type="button" onClick={() => setIsRegistering(!isRegistering)} className="w-full text-xs font-bold text-stone-800 uppercase mt-4 text-center underline">
  ...
  const menuItems = userRole === 'farmer' ? [
    ...
    { id: 'treatments', label: 'Libretto Sanitario', icon: Syringe },
  ...
  const overdueTreatments = expiringTreatments.filter(t => t.daysLeft < 0);
  const upcomingTreatments = expiringTreatments.filter(t => t.daysLeft >= 0);
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col md:flex-row relative text-stone-800 font-sans overflow-x-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r p-6 fixed h-full shadow-sm z-40">
        ...
        <nav className="space-y-1 flex-1 overflow-y-auto custom-scrollbar">
          {menuItems.map(item => (
            <button 
              ...
              className={`flex items-center gap-3 w-full p-3 rounded-xl font-bold text-sm transition-all ${activeTab === item.id ? 'bg-emerald-600 text-white shadow-md' : 'text-stone-800 hover:bg-stone-50'}`}
            >
              <item.icon size={18} /> 
              <span>{item.label}</span>
              {item.id === 'treatments' && expiringTreatments.length > 0 && (
                <span className="ml-auto bg-red-500 text-white text-[8px] w-5 h-5 rounded-full flex items-center justify-center">
        ...
        <button onClick={() => signOut(auth)} className="mt-4 flex items-center gap-2 text-red-600 font-bold p-2 text-xs uppercase">
          <LogOut size={18} /> Esci
        </button>
      </aside>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-xl border-t flex justify-around p-2 z-50 shadow-lg overflow-x-auto hide-scrollbar">
        {menuItems.map(item => (
          <button 
            key={item.id} 
            onClick={() => setActiveTab(item.id)} 
            className={`flex flex-col items-center min-w-[65px] p-2 transition-colors relative ${activeTab === item.id ? 'text-emerald-600' : 'text-stone-500'}`}
          >
            <item.icon size={22} />
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 pb-24 pt-safe">
        <div className="mb-6 flex justify-between items-center">
          <h2 className="text-xl font-black text-stone-900 uppercase italic tracking-tight">
            {menuItems.find(i => i.id === activeTab)?.label}
          </h2>
          <div className="flex items-center gap-2">
            {!oneSignalId && (
              <button
                onClick={() => window.OneSignal?.showSlidedownPrompt()}
                className="bg-amber-500 text-white p-2 rounded-full shadow-md animate-pulse"
        ...
        {showAssistant && (
          <div className="mb-6 bg-white p-4 rounded-2xl border-2 border-blue-100 shadow-xl animate-in slide-in-from-top-4">
            ...
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleAICommand} className="bg-blue-600 text-white px-5 rounded-xl font-bold text-xs shadow-md">
                OK
              </button>
            </div>
            {aiLogs.length > 0 && (
            ...
            {expiringTreatments.length > 0 && (
              <div className="bg-amber-50 border border-amber-200 rounded-3xl p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-black text-amber-800 uppercase">Trattamenti in Scadenza</h3>
                </div>
                ...
                {overdueTreatments.length > 0 && (
                  <div className="mb-4">
                    ...
                    <div className="space-y-2">
                      {overdueTreatments.map(t => (
                        <div key={t.treatment.id} className="bg-white p-3 rounded-xl border-l-4 border-l-red-500">
                          <div className="flex justify-between items-start">
                            <div>
                              <p className="font-bold text-stone-800">{t.animalName}</p>
                              <p className="text-xs text-stone-600">{t.treatment.tipo}</p>
                            </div>
                            <span className="text-red-600 font-black text-xs bg-red-50 px-2 py-1 rounded-full">
                              SCADUTO
                            </span>
                          </div>
                        </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {upcomingTreatments.map(t => (
                        <div key={t.treatment.id} className="bg-white p-3 rounded-xl border-l-4 border-l-amber-400">
                          <div className="flex justify-between items-start">
                            <div>
                              <p className="font-bold text-stone-800">{t.animalName}</p>
                              <p className="text-xs text-stone-600">{t.treatment.tipo}</p>
                            </div>
                            <span className="text-amber-600 font-black text-xs bg-amber-50 px-2 py-1 rounded-full">
        ...
        {/* LIBRETTO SANITARIO */}
        {activeTab === 'treatments' && userRole === 'farmer' && (
          <div className="space-y-6 animate-fade-in">
            <div className="bg-white p-5 rounded-3xl border shadow-sm">
              <h3 className="text-sm font-black text-stone-900 uppercase mb-4 flex items-center gap-2">
            ...
            {selectedAnimal && (
              <>
                <div className="bg-white p-5 rounded-3xl border shadow-sm">
                  <h4 className="text-sm font-black text-stone-900 uppercase mb-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="text-xs font-bold text-stone-700 block mb-1">
                        Data Scadenza/Richiamo <span className="text-stone-500 font-normal">(opzionale)</span>
                      </label>
                      <input
                        type="date"
                        className="w-full p-3 bg-stone-50 rounded-xl font-bold text-stone-800 text-sm border-none shadow-inner"
                ...
                <div className="bg-white p-5 rounded-3xl border shadow-sm">
                  <h4 className="text-sm font-black text-stone-900 uppercase mb-4 flex items-center gap-2">
                          ...
                          return (
                            ...
                            >
                              <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-2">
                                  <span className="font-black text-stone-900">{t.tipo}</span>
                                  {t.completed && (
                                    <span className="text-emerald-600 text-[8px] bg-emerald-100 px-2 py-0.5 rounded-full">
                                      COMPLETATO
                                    </span>
                                  )}
                                  {isExpired && (
                              ...
                              <div className="grid grid-cols-2 gap-2 text-xs">
                                ...
                                {t.dataScadenza && (
                                  <div>
                                    <span className="text-stone-500">Scadenza:</span>
                              ...
                              {t.note && (
                                <p className="mt-2 text-xs text-stone-600 italic bg-white p-2 rounded-lg">
                                  {t.note}
                                </p>
                              )}
                            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border shadow-xl max-w-lg mx-auto border-t-8 border-blue-600">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {['Febbre', 'Tosse', 'Diarrea', 'Zoppia', 'Gonfiore', 'Ferita'].map(s => (
                    <button
                      key={s}
                      type="button"
            ...
            >
              {isAnalyzing ? (
                ...
                <><Stethoscope size={16}/> ANALIZZA SINTOMI</>
              )}
            </button>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            <div className="bg-white p-4 rounded-2xl border shadow-sm space-y-3">
              <div className="flex justify-between items-center border-b pb-2">
                ...
                <button onClick={exportASLReport} className="text-[9px] font-bold bg-stone-900 text-white px-3 py-1 rounded-lg uppercase shadow-md">
                  PDF ASL
                </button>
              </div>
                    ...
                    {isExpanded && (
                      <div className="p-4 border-t border-stone-100 bg-stone-50/50">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                          {capi.map(a => (
                            <div key={a.id} className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm relative group hover:border-emerald-500">
                              <div className="flex justify-between items-start mb-1">
                                <h4 className="font-black text-stone-800 uppercase text-xs">{a.name}</h4>
                                <div className="flex gap-2">
                                  <button onClick={()=>{setEditingAnimalId(a.id); setEditNote(a.notes || '');}} className="text-stone-600 hover:text-emerald-500">
                                    <Edit2 size={14}/>
                                  </button>
                                  <button onClick={()=>deleteDoc(doc(db,'animals',a.id))} className="text-stone-600 hover:text-red-500">
                                    <Trash2 size={14}/>
                                  </button>
                                </div>
                              </div>
        ...
        {activeTab === 'finance' && userRole === 'farmer' && (
          <div className="space-y-6 animate-in slide-in-from-right-4">
            <div className="bg-white p-4 rounded-2xl border shadow-sm grid grid-cols-1 md:grid-cols-5 gap-2">
              ...
              <select 
                className={`p-2 rounded-lg font-black text-[10px] border-none shadow-sm uppercase ${newTrans.type === 'Entrata' ? 'bg-emerald-600 text-white' : 'bg-red-600 text-white'}`} 
                value={newTrans.type} 
              ...
              <button 
                ...
                className="bg-emerald-950 text-white rounded-lg py-3 text-[10px] font-black uppercase col-span-full shadow-lg mt-1 active:scale-95"
              >
                Registra Movimento
              </button>
            </div>
                      ...
                      <div key={t.id} className="bg-white p-3 rounded-xl border border-stone-200 flex justify-between items-center shadow-sm">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[8px] font-bold text-stone-500 uppercase">{t.date}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="bg-gradient-to-r from-amber-500 to-orange-500 p-8 rounded-3xl text-white shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                <h3 className="text-3xl font-black italic uppercase mb-2">Market Km 0</h3>
                <p className="text-amber-100 text-sm max-w-md">
                  Acquista e vendi prodotti locali
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {marketItems.length === 0 ? (
                <div className="col-span-full text-center py-12 bg-white rounded-3xl border border-stone-200">
                  ...
                  <p className="text-stone-600 font-bold text-sm">Nessun prodotto in vendita</p>
                  {userRole === 'farmer' && (
                    <button 
                      onClick={() => setActiveTab('products')}
                      className="mt-4 bg-emerald-600 text-white px-6 py-2 rounded-xl text-xs font-black uppercase"
                    >
                      Vai alle tue scorte
                    </button>
                  )}
                ...
                marketItems.map(item => (
                  <div key={item.id} className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-sm hover:shadow-lg transition-all hover:scale-[1.02]">
                    <div className="h-32 bg-gradient-to-br from-amber-100 to-orange-100 flex items-center justify-center relative">
                      <ShoppingBag size={48} className="text-amber-600 opacity-50" />
                      <div className="absolute top-3 right-3 bg-amber-500 text-white px-3 py-1 rounded-full text-sm font-black">
                    ...
                    <div className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          <p className="text-xs text-stone-600">da {item.sellerName}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        {item.contactPhone ? (
                          <a 
                            href={`https://wa.me/39${item.contactPhone}?text=Salve ${item.sellerName}, sono interessato a ${item.name}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex-1 bg-[#25D366] text-white py-3 rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 hover:bg-[#128C7E]"
                          >
                          ...
                          <a 
                            ...
                            className="flex-1 bg-blue-600 text-white py-3 rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 hover:bg-blue-700"
                          >
                            <Mail size={16} />
                            Email
                          </a>
                        ...
                        {user?.uid !== item.sellerId && (
                          <button
                            onClick={() => {
                              if (confirm(`Confermi l'acquisto di ${item.quantity} ${item.unit} di ${item.name} per €${(item.price * item.quantity).toFixed(2)}?`)) {
                                handlePurchase(item);
                              }
                            }}
                            className="px-4 bg-emerald-600 text-white py-3 rounded-xl text-xs font-black uppercase hover:bg-emerald-700"
                          >
                            Acquista
                          </button>
                        )}
                        ...
                        {user?.uid === item.sellerId && (
                          <button
                            onClick={() => deleteDoc(doc(db, 'market_items', item.id))}
                            className="px-4 bg-red-500 text-white py-3 rounded-xl text-xs font-black uppercase hover:bg-red-600"
                          >
                            <Trash2 size={16} />
                          </button>
                        )}
        ...
        {activeTab === 'dinastia' && (
          <div className="bg-white p-8 rounded-3xl border shadow-sm overflow-x-auto">
            <h3 className="text-xl font-black italic uppercase mb-8 flex items-center gap-2 text-stone-900">
