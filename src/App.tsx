...
export default function App() {
  ...
  // CONTEGGIO REALE (SOLO CAPI CON NOME)
  ...
  // FUNZIONE RIDUCI PRODOTTO (RISOLVE IMMAGINE 2)
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black text-emerald-800 bg-stone-50 italic animate-pulse text-2xl">CARICAMENTO...</div>;
        ...
        {capiFantasma > 0 && (
          <div className="mb-8 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-between">
            <div className="flex items-center gap-3 text-amber-700 font-bold">
              <Ghost size={20} />
              <span>Rilevati capi vuoti. Premi per pulire il conteggio:</span>
            </div>
          ...
          <div className="space-y-12">
            <div className="bg-white p-10 rounded-[2.5rem] border border-stone-100 shadow-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <select className="ui-input w-full text-xs" value={newAnimal.sire} onChange={e => setNewAnimal({...newAnimal, sire: e.target.value})}><option value="">Padre (Ignoto)</option>{capiVeri.filter(a => a.species === newAnimal.species).map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select>
                <select className="ui-input w-full text-xs" value={newAnimal.dam} onChange={e => setNewAnimal({...newAnimal, dam: e.target.value})}><option value="">Madre (Ignota)</option>{capiVeri.filter(a => a.species === newAnimal.species).map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select>
              </div>
              <textarea placeholder="Note sanitarie o alimentari..." className="ui-input w-full h-24 resize-none mb-8" value={newAnimal.notes} onChange={e => setNewAnimal({...newAnimal, notes: e.target.value})}></textarea>
              ...
              return capi.length > 0 && (
                <div key={species} className="space-y-6">
                  <div className="flex items-center gap-4">
                      ...
                      <span className="bg-white border px-4 py-1.5 rounded-full text-xs font-black text-emerald-700">{capi.length} INDIVIDUI</span>
                  </div>
                      ...
                      <div key={a.id} className="bg-white p-8 rounded-[2rem] border border-stone-100 relative group shadow-sm hover:shadow-xl transition-all duration-300">
                        {editingId === a.id ? (
                          ...
                          <>
                            ...
                            <p className="text-[10px] text-stone-400 font-black uppercase mb-4 flex items-center gap-2 tracking-widest"><CalendarDays size={14} /> Nato: {a.birthDate || 'N/D'}</p>
        ...
        {activeTab === 'finance' && (
          <div className="space-y-12">
                ...
                return (
                    <div key={s} className="bg-white p-8 rounded-[2.5rem] border shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <span className={`text-3xl font-black ${balance >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>€ {balance.toFixed(2)}</span>
                        </div>
                        <div className="space-y-2">
                            {specTrans.sort((a,b) => b.date.localeCompare(a.date)).map(t => (
                                <div key={t.id} className="p-5 flex justify-between items-center hover:bg-stone-50 transition group rounded-xl">
                                  <div className="flex items-center gap-4">
                                  ...
                                  <div className="flex items-center gap-6">
                                    <span className={`font-black text-lg ${t.type === 'Entrata' ? 'text-emerald-600' : 'text-red-500'}`}>
                                      {t.type === 'Entrata' ? '+' : '-'} €{t.amount.toFixed(2)}
                                    </span>
                                    <button onClick={async () => { if(confirm("Cancellare?")) await deleteDoc(doc(db, 'transactions', t.id)); }} className="text-stone-300 hover:text-red-600 p-2 rounded-xl transition-colors md:opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button>
                                  </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="bg-white p-10 rounded-[2.5rem] border shadow-xl">
              ...
              <button onClick={async () => { if(!newTrans.desc) return; await addDoc(collection(db, 'transactions'), { ...newTrans, date: new Date().toLocaleDateString('it-IT'), ownerId: user.uid }); setNewTrans({desc:'', amount:0, type:'Entrata', species:'Maiali'}); }} className="bg-emerald-600 text-white font-black rounded-2xl py-4 mt-6 px-12 shadow-lg hover:bg-emerald-700 transition-all uppercase text-[10px]">Registra</button>
            </div>
          </div>
        )}
