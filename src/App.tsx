...
const DynastyBranch = ({ animal, allAnimals, level = 0 }: { animal: Animal, allAnimals: Animal[], level?: number }) => {
  ...
  return (
    <div className={level > 0 ? "ml-4 border-l border-stone-200 pl-4 mt-2" : ""}>
      <div className={`p-3 rounded-xl border bg-white mb-2 shadow-sm ${level === 0 ? 'border-l-4 border-emerald-500' : ''}`}>
        <p className="font-bold text-stone-800 text-xs uppercase">{animal.name}</p>
        <p className="text-[9px] text-stone-400 font-bold">{animal.species} • GEN {level}</p>
      </div>
...
export default function App() {
      ...
      if (u) {
        ...
        try {
          ...
          if (userDoc && userDoc.exists()) {
            ...
            setActiveTab(userDoc.data().role === 'consumer' ? 'market' : 'dashboard');
  ...
  const handleSaveBirth = async () => {
    if (!newBirth.idCode.trim()) return alert("ID Madre richiesto.");
  ...
  const handlePublishToMarket = async () => {
    ...
    alert("Prodotto pubblicato nel mercato!");
  };
      ...
      <div className="min-h-screen bg-stone-100 flex items-center justify-center p-4">
        <div className="bg-white p-6 rounded-3xl shadow-xl w-full max-w-sm border">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <button type="button" onClick={() => setIsRegistering(!isRegistering)} className="w-full text-xs font-bold text-stone-400 uppercase mt-4 text-center">{isRegistering ? "Accedi" : "Registrati qui"}</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col md:flex-row relative">
      ...
      {/* BOTTONE IA */}
      {userRole === 'farmer' && (
        <button onClick={() => setShowAssistant(!showAssistant)} className="fixed bottom-20 right-4 md:bottom-8 md:right-8 bg-blue-600 text-white p-4 rounded-full shadow-lg z-50">
          <Bot size={24} />
        </button>
      )}
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-10 pb-24">
        <div className="mb-6 flex justify-between items-center">
            ...
            <p className="text-[10px] font-bold text-stone-400 uppercase bg-white px-3 py-1 rounded-full border shadow-sm">Utente: {userName}</p>
        </div>

        {/* MODALE IA */}
        {showAssistant && (
          <div className="mb-6 bg-white p-5 rounded-2xl border-2 border-blue-100 shadow-xl animate-in slide-in-from-top-4">
            <h3 className="text-blue-900 font-bold text-xs uppercase mb-3 flex items-center gap-2 italic"><Bot size={16}/> Assistente Vocale</h3>
            <div className="flex gap-2">
        ...
        {activeTab === 'dashboard' && userRole === 'farmer' && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div onClick={()=>setActiveTab('inventory')} className="bg-emerald-600 p-5 rounded-3xl text-white shadow-lg cursor-pointer">
              <p className="text-[10px] font-bold uppercase opacity-60">Capi in Stalla</p>
            ...
            <div onClick={()=>setActiveTab('tasks')} className="bg-stone-900 p-5 rounded-3xl text-white shadow-lg cursor-pointer">
               <p className="text-[10px] font-bold text-emerald-400 uppercase">Agenda</p>
               <h4 className="text-2xl font-black">{tasks.filter(t=>!t.done).length}</h4>
            </div>
          </div>
        )}

        {/* --- 2. INVENTARIO (RAGGRUPPATO PER SPECIE) --- */}
        {activeTab === 'inventory' && userRole === 'farmer' && (
          <div className="space-y-8 animate-in slide-in-from-bottom-4">
            <div className="bg-white p-5 rounded-2xl border shadow-sm">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xs font-black uppercase text-stone-400 italic">Nuovo Capo</h3>
                <button onClick={exportASLReport} className="text-[10px] font-bold bg-stone-100 px-3 py-1 rounded-lg flex items-center gap-1 uppercase hover:bg-emerald-600 hover:text-white transition-all"><FileDown size={14}/> PDF</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <input placeholder="ID Capo" className="p-3 bg-stone-50 rounded-xl text-sm border-none shadow-inner" value={newAnimal.name} onChange={e=>setNewAnimal({...newAnimal, name:e.target.value})} />
                ...
                <button onClick={handleSaveAnimal} className="bg-emerald-600 text-white font-bold rounded-xl p-4 uppercase text-[10px] shadow-lg">Aggiungi</button>
              </div>
            </div>
            ...
            {speciesList.map(specie => {
              ...
              return (
                <div key={specie} className="space-y-4 animate-in fade-in">
                  <h4 className="text-sm font-black text-emerald-800 uppercase px-3 bg-emerald-50 w-fit py-1 rounded-lg italic">{specie} ({capi.length})</h4>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    {capi.map(a => (
                      <div key={a.id} className="bg-white p-5 rounded-2xl border border-stone-100 shadow-sm relative group hover:border-emerald-500 transition-all">
                        ...
                        <h4 className="text-lg font-black text-stone-900 leading-tight uppercase">{a.name}</h4>
        ...
        {activeTab === 'births' && userRole === 'farmer' && (
          <div className="bg-white p-8 rounded-3xl border shadow-sm max-w-lg mx-auto animate-in zoom-in-95">
             <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100 text-amber-700 mb-6 flex items-center gap-3">
               <Baby size={32} />
               <p className="text-[11px] font-bold italic uppercase tracking-tighter">Registra il parto: i nati verranno aggiunti automaticamente all'anagrafica stalla.</p>
             </div>
             <div className="space-y-4">
                <input className="w-full p-3 bg-stone-50 rounded-xl font-bold text-sm shadow-inner border-none" placeholder="Codice Madre" value={newBirth.idCode} onChange={e=>setNewBirth({...newBirth, idCode:e.target.value})} />
                <div className="flex gap-2">
                ...
                <button onClick={handleSaveBirth} className="w-full bg-emerald-700 text-white py-4 rounded-xl font-black uppercase text-xs shadow-lg">Registra Ora</button>
             </div>
          </div>
        )}
        ...
        {activeTab === 'finance' && userRole === 'farmer' && (
          <div className="space-y-6 animate-in slide-in-from-right-4">
            <div className="bg-stone-900 p-8 rounded-3xl text-white shadow-xl text-center">
               <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mb-2">Netto Aziendale</p>
            ...
            <div className="bg-white p-5 rounded-2xl border grid grid-cols-1 md:grid-cols-4 gap-3 shadow-sm">
              <input placeholder="Causale Spesa/Entrata" className="p-3 bg-stone-50 rounded-xl font-bold text-sm shadow-inner border-none col-span-1 md:col-span-2" value={newTrans.desc} onChange={e=>setNewTrans({...newTrans, desc:e.target.value})} />
            ...
            {speciesList.map(specie => {
              ...
              return (
                <div key={specie} className="space-y-2 animate-in fade-in">
                  <div className="flex justify-between items-center px-4 py-1 bg-stone-50 rounded-xl">
                  ...
                  <div className="space-y-2">
                    {transSpecie.map(t => (
                      <div key={t.id} className="bg-white p-4 rounded-2xl border border-stone-100 flex justify-between items-center shadow-sm">
        ...
        {activeTab === 'products' && userRole === 'farmer' && (
          <div className="space-y-8">
            {sellingProduct && (
              <div className="bg-amber-50 p-6 rounded-3xl border-2 border-amber-300 shadow-xl animate-in zoom-in-95 max-w-lg mx-auto">
                ...
                <div className="flex gap-4">
                  <button onClick={handlePublishToMarket} className="flex-1 bg-amber-500 text-white font-bold py-3 rounded-xl uppercase text-[10px] tracking-widest shadow-xl">Conferma</button>
            ...
            <div className="bg-white p-6 rounded-3xl border shadow-sm relative overflow-hidden">
              <h3 className="text-[10px] font-black uppercase text-stone-400 mb-6 tracking-widest flex items-center gap-2 italic"><Package size={16}/> Carico e Scarico Scorte</h3>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {products.map(p => (
                <div key={p.id} className="bg-white p-5 rounded-3xl border border-stone-100 shadow-sm text-center flex flex-col group hover:border-emerald-200 transition-all duration-500">
                  ...
                  <h4 className="font-bold text-stone-900 uppercase text-[10px] mb-2 tracking-widest">{p.name}</h4>
                  ...
                  <button onClick={()=>setSellingProduct(p)} className="w-full bg-amber-100 text-amber-700 font-bold py-3 rounded-2xl text-[10px] uppercase tracking-widest mb-3 hover:bg-amber-500 hover:text-white transition-all flex items-center justify-center gap-2 italic"><Store size={14}/> Vendi Km 0</button>
        ...
        {activeTab === 'tasks' && userRole === 'farmer' && (
          <div className="max-w-xl space-y-6 animate-in slide-in-from-left-6 mx-auto">
            <div className="bg-white p-6 rounded-3xl border shadow-sm space-y-5">
               <h3 className="text-[11px] font-black uppercase text-stone-400 mb-2 tracking-[0.4em] italic flex items-center gap-3 leading-none"><CalendarDays className="text-emerald-600" size={18}/> Pianificazione Lavori</h3>
               <div className="flex flex-col md:flex-row gap-2">
                 <input className="flex-1 p-3 bg-stone-50 rounded-xl font-bold text-sm shadow-inner border-none" placeholder="Cosa devi segnare?" value={newTask} onChange={e=>setNewTask(e.target.value)} />
                 <input type="date" className="p-3 bg-stone-50 rounded-xl font-bold text-xs uppercase tracking-widest text-emerald-700 shadow-inner border-none" value={newTaskDate} onChange={e=>setNewTaskDate(e.target.value)} />
                 <button onClick={handleAddTask} className="bg-emerald-600 text-white px-8 rounded-xl font-bold text-xs uppercase">Add</button>
               </div>
               {/* BARRA RICERCA */}
               <div className="relative group">
                  ...
                  <input className="w-full p-3 pl-10 bg-stone-100 border-none rounded-xl font-bold text-xs uppercase italic tracking-widest shadow-inner focus:ring-2 focus:ring-emerald-50" placeholder="Filtra lavori (es: vaccino, fieno)..." value={taskSearch} onChange={e=>setTaskSearch(e.target.value)} />
               </div>
            </div>
            <div className="space-y-3">
              {tasks.filter(t => t.text.toLowerCase().includes(taskSearch.toLowerCase()))
                ...
                <div key={t.id} className={`bg-white p-5 rounded-3xl border-2 flex justify-between items-center transition-all ${t.done ? 'opacity-30 grayscale' : 'shadow-sm border-white hover:border-emerald-500 group border-l-8 border-l-emerald-600'}`}>
                  <div>
                    ...
                    <p className="text-[10px] font-black text-stone-300 uppercase tracking-widest">Scadenza: <span className="text-emerald-600">{t.dueDate || 'N/D'}</span></p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={()=>updateDoc(doc(db,'tasks',t.id),{done:!t.done})} className={`p-3 rounded-2xl shadow-sm transition-all ${t.done ? 'bg-stone-100 text-stone-400' : 'bg-emerald-50 text-emerald-600'}`}><CheckCircle2 size={24}/></button>
              ...
              {tasks.length === 0 && <p className="text-center py-20 text-stone-200 font-black uppercase italic tracking-widest text-xs leading-none">Nessun impegno in agenda.</p>}
            </div>
          </div>
        )}
        ...
        {activeTab === 'dinastia' && userRole === 'farmer' && (
          <div className="bg-white p-8 rounded-3xl border shadow-sm animate-in fade-in duration-1000 relative overflow-hidden overflow-x-auto">
            <div className="flex items-center gap-4 mb-12 border-b-4 border-stone-50 pb-8">
               ...
               <div>
                  <h3 className="text-3xl font-black italic uppercase tracking-tighter leading-none mb-1 text-stone-900">Albero Genealogico</h3>
                  <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest italic leading-none">Linea di sangue e genetica</p>
               </div>
            </div>
        ...
        {activeTab === 'vet' && userRole === 'farmer' && (
          <div className="bg-white p-8 rounded-[4rem] border shadow-2xl max-w-2xl animate-in zoom-in-95 duration-700 mx-auto relative overflow-hidden">
            <div className="bg-blue-600 p-8 rounded-[2.5rem] text-white mb-10 flex items-center gap-6 shadow-2xl shadow-blue-100">
               ...
               <div className="relative z-10">
                  ...
                  <p className="text-blue-50 font-bold opacity-80 text-xs leading-relaxed italic max-w-lg tracking-tight">Triage clinico istantaneo basato su dati veterinari 2026.</p>
               </div>
            </div>
            <textarea className="w-full p-8 bg-stone-50 border-none rounded-[2.5rem] font-bold text-stone-800 text-lg h-64 shadow-inner italic placeholder:text-stone-300 focus:ring-8 focus:ring-blue-50 transition-all leading-snug" placeholder="Descrivi il comportamento anomalo del capo..." value={vetSymptom} onChange={e=>setVetSymptom(e.target.value)}></textarea>
            <button onClick={()=>{setIsAnalyzing(true); setTimeout(()=>{setVetResult({title:"Esito Diagnostico", desc:"I sintomi indicati suggeriscono una possibile spossatezza da calore o fase iniziale di infezione respiratoria.", action:"ISOLARE IL CAPO, MONITORARE LA TEMPERATURA E CONTATTARE IL VETERINARIO DI ZONA URGENTEMENTE"}); setIsAnalyzing(false);},3000)}} className="w-full bg-stone-950 text-white py-6 rounded-[2rem] font-black uppercase tracking-[0.5em] text-xs flex items-center justify-center gap-8 hover:bg-blue-600 transition-all shadow-2xl">
              {isAnalyzing ? <><Activity className="animate-spin" size={28}/> Neural Processing...</> : <><Bot size={36}/> Avvia Triage</>}
            </button>
            {vetResult && (
              <div className="mt-16 p-10 bg-emerald-50 border-4 border-emerald-100 rounded-[3.5rem] animate-in slide-in-from-bottom-12 duration-700">
                 ...
                 <p className="text-emerald-800 font-bold text-xl mb-10 leading-tight italic tracking-tighter leading-relaxed">"{vetResult.desc}"</p>
        ...
        {activeTab === 'market' && (
          <div className="space-y-16 animate-in slide-in-from-bottom-12 duration-1000">
            <div className="bg-amber-500 p-12 md:p-20 rounded-[4rem] text-white shadow-[0_50px_100px_rgba(245,158,11,0.2)] relative overflow-hidden">
               ...
               <div className="relative z-10 max-w-2xl text-center md:text-left mx-auto md:mx-0">
                 <h3 className="text-5xl font-black italic tracking-tighter uppercase mb-6 leading-none">Piazza del Mercato</h3>
                 <p className="text-amber-50 font-black text-xl leading-relaxed opacity-95 italic tracking-tighter">Sostieni l'agricoltura locale. Filiera corta digitale, trasparente, sicura.</p>
               </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {marketItems.map(item => (
                <div key={item.id} className="bg-white rounded-[3.5rem] border-2 border-stone-50 shadow-[0_30px_80px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col group hover:shadow-2xl hover:translate-y-[-10px] transition-all duration-700">
                  <div className="h-56 bg-stone-50 flex items-center justify-center relative overflow-hidden group-hover:bg-amber-50 transition-colors duration-1000 shadow-inner">
                     <div className="absolute top-6 right-6 bg-white/95 backdrop-blur-3xl px-5 py-2.5 rounded-[1.5rem] font-black text-xl text-emerald-600 shadow-2xl border-2 border-emerald-50 italic tracking-tighter group-hover:scale-110 transition-transform z-20 leading-none">€{item.price.toFixed(0)}</div>
                  ...
                  <div className="p-10 flex flex-col flex-1 relative">
                    ...
                    <h4 className="text-2xl font-black text-stone-950 tracking-tighter mb-8 leading-[0.85] uppercase group-hover:text-amber-600 transition-colors duration-500">{item.name}</h4>
                    <div className="flex justify-between items-center mb-10 bg-stone-50 p-6 rounded-3xl border-2 border-white shadow-inner">
                       <span className="text-[11px] font-black text-stone-400 uppercase italic tracking-widest leading-none">Disp.</span>
                    ...
                    {item.contactPhone ? (
                       <a href={`https://wa.me/39${item.contactPhone}?text=Salve ${item.sellerName}, vorrei ordinare ${item.name} visto su AgriManage.`} target="_blank" rel="noreferrer" className="w-full bg-[#25D366] text-white py-6 rounded-3xl font-black text-xs uppercase tracking-[0.4em] shadow-[0_15px_40px_rgba(37,211,102,0.3)] flex items-center justify-center gap-4 hover:scale-105 transition-all active:scale-95"><MessageCircle size={24}/> WhatsApp</a>
                    ) : (
                       <a href={`mailto:${item.contactEmail}`} className="w-full bg-blue-600 text-white py-6 rounded-3xl font-black text-xs uppercase tracking-[0.4em] shadow-2xl shadow-blue-200 flex items-center justify-center gap-4 hover:scale-105 transition-all active:scale-95"><Mail size={24}/> Email</a>
                    )}
                  </div>
                </div>
              ))}
