...
import { 
  ...
  PlusCircle, LogOut, Menu, X, Search, History, Package, Edit2, 
  CheckCircle2, MinusCircle, Activity, ListChecks, Wallet, 
...
export default function App() {
  ...
  const capiFantasma = animals.length - capiVeri.length;
  ...
  useEffect(() => {
    ...
    const uA = onSnapshot(qF('animals'), s => setAnimals(s.docs.map(d => ({id: d.id, ...d.data()})) as Animal[]));
    const uB = onSnapshot(qF('births'), s => setBirths(s.docs.map(d => ({id: d.id, ...d.data()})) as BirthRecord[]));
    const uT = onSnapshot(qF('transactions'), s => setTransactions(s.docs.map(d => ({id: d.id, ...d.data()})) as Transaction[]));
    const uTk = onSnapshot(qF('tasks'), s => setTasks(s.docs.map(d => ({id: d.id, ...d.data()})) as Task[]));
    const uP = onSnapshot(qF('products'), s => setProducts(s.docs.map(d => ({id: d.id, ...d.data()})) as Product[]));
    return () => { uA(); uB(); uT(); uTk(); uP(); };
  }, [user]);

  const pulisciDatabase = async () => {
    const q = query(collection(db, 'animals'), where("ownerId", "==", user?.uid));
    const snap = await getDocs(q);
    snap.forEach(async (d) => { if (!d.data().name) await deleteDoc(doc(db, 'animals', d.id)); });
  };
  ...
  const handleSaveBirth = async () => {
    ...
    for (let i = 0; i < newBirth.count; i++) {
      await addDoc(collection(db, 'animals'), { name: `Figlio di ${newBirth.idCode} #${i + 1}`, species: newBirth.species, birthDate: newBirth.birthDate, dam: newBirth.idCode, notes: 'Parto auto', ownerId: user?.uid });
    }
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black text-emerald-800 bg-stone-50 animate-pulse">AGRIMANAGE PRO...</div>;
    ...
    return (
      <div className="min-h-screen bg-emerald-950 flex items-center justify-center p-4">
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-w-md">
          ...
          <form onSubmit={async (e) => { e.preventDefault(); try { await signInWithEmailAndPassword(auth, email, password); } catch(err) { alert("Dati errati"); } }} className="space-y-4">
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#F8F9FA] text-stone-900">
      ...
      <main className="flex-1 p-6 md:p-14 h-screen overflow-y-auto scroll-smooth">
        ...
        {capiFantasma > 0 && (
          <div className="mb-8 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-between">
            <div className="flex items-center gap-3 text-amber-700 font-bold"><Ghost size={20} /><span>Dati sporchi rilevati. Premi per pulire:</span></div>
        ...
        <h2 className="text-5xl font-black text-emerald-950 capitalize tracking-tighter italic mb-10">{activeTab}</h2>

        {activeTab === 'inventory' && (
          <div className="space-y-12">
            <div className="bg-white p-10 rounded-[2.5rem] border border-stone-100 shadow-xl">
              <h3 className="text-xs font-black mb-8 text-emerald-900 uppercase flex items-center gap-2"><PlusCircle size={20} /> Nuovo Capo</h3>
              ...
              <textarea placeholder="Note sanitarie..." className="ui-input w-full h-24 mb-8" value={newAnimal.notes} onChange={e => setNewAnimal({...newAnimal, notes: e.target.value})}></textarea>
              <button onClick={async () => { if(!newAnimal.name) return; await addDoc(collection(db, 'animals'), { ...newAnimal, ownerId: user.uid }); setNewAnimal({name:'', species:'Maiali', birthDate:'', sire:'', dam:'', notes:''}); }} className="bg-emerald-600 text-white font-black rounded-2xl py-5 px-12 uppercase text-xs">Salva</button>
            </div>
              ...
              return capi.length > 0 && (
                <div key={species} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {capi.map(a => (
                      <div key={a.id} className="bg-white p-8 rounded-[2rem] border border-stone-100 relative group shadow-sm hover:shadow-xl">
                        <h4 className="text-2xl font-black text-stone-800">{a.name}</h4>
                        <p className="text-[10px] text-stone-400 font-black uppercase mb-4">{a.birthDate || 'N/D'}</p>
                        {a.notes && <div className="mt-4 p-4 bg-stone-50 rounded-2xl text-xs italic border-l-4 border-emerald-500">{a.notes}</div>}
        ...
        {activeTab === 'finance' && (
          <div className="space-y-12">
                ...
                return (
                    <div key={s} className="bg-white p-8 rounded-[2.5rem] border shadow-sm">
                        ...
                        <div className="space-y-2">
                            {specTrans.map(t => (
                                <div key={t.id} className="flex justify-between items-center p-3 bg-stone-50 rounded-xl group">
                                    <div className="flex items-center gap-4">
                                      <div className={t.type === 'Entrata' ? 'text-emerald-500' : 'text-red-500'}>{t.type === 'Entrata' ? <ArrowUpRight size={18} /> : <ArrowDownLeft size={18} />}</div>
                                      <span className="font-bold text-stone-700">{t.desc} ({t.date})</span>
                                    </div>
                                    <div className="flex items-center gap-6">
                                      <span className={t.type === 'Entrata' ? 'text-emerald-600 font-black' : 'text-red-500 font-black'}>€{t.amount.toFixed(2)}</span>
                                      <button onClick={async () => { if(window.confirm("Eliminare?")) await deleteDoc(doc(db, 'transactions', t.id)); }} className="text-stone-300 hover:text-red-600"><Trash2 size={18} /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="bg-white p-10 rounded-[2.5rem] border shadow-xl">
              ...
              <button onClick={async () => { if(!newTrans.desc) return; await addDoc(collection(db, 'transactions'), { ...newTrans, date: new Date().toLocaleDateString('it-IT'), ownerId: user.uid }); setNewTrans({desc:'', amount:0, type:'Entrata', species:'Maiali'}); }} className="bg-emerald-600 text-white font-black rounded-2xl py-4 mt-6 px-12 uppercase text-[10px]">Salva</button>
            </div>
          </div>
        )}
            ...
            {speciesList.map(species => {
              ...
              return founders.length > 0 && (
                <div key={species} className="bg-white p-10 rounded-[3rem] border shadow-xl">
                  ...
                  {founders.map(founder => <div key={founder.id} className="mb-10 last:mb-0"><DynastyBranch animal={founder} allAnimals={capiVeri} level={0} /></div>)}
                </div>
              );
            })}
          </div>
        ...
        {activeTab === 'products' && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {products.map(p => (
              <div key={p.id} className="bg-white p-8 rounded-[2rem] border text-center shadow-sm relative group">
                ...
                <div className="inline-block bg-emerald-600 text-white px-6 py-2 rounded-full text-2xl font-black mb-6">{p.quantity} <span className="text-xs font-bold">{p.unit}</span></div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
            <div className="bg-white p-10 rounded-[3rem] border shadow-xl">
              ...
              <div className="flex gap-4 mb-10">
                ...
                <button onClick={async () => { if(newTask) { await addDoc(collection(db, 'tasks'), { text: newTask, done: false, ownerId: user.uid }); setNewTask(''); } }} className="bg-emerald-600 text-white px-8 rounded-2xl font-black uppercase text-[10px]">Aggiungi</button>
              </div>
              <div className="space-y-3">
                {tasks.filter(t => !t.done).map(t => (
                  <div key={t.id} className="flex items-center justify-between p-6 bg-stone-50 rounded-[1.5rem] border">
                    <div className="flex items-center gap-6"><button onClick={async () => await updateDoc(doc(db, 'tasks', t.id), { done: true, dateCompleted: new Date().toLocaleDateString('it-IT') })} className="text-stone-300 hover:text-emerald-500"><CheckCircle2 size={32}/></button><span className="font-black text-stone-700">{t.text}</span></div>
                    <button onClick={() => deleteDoc(doc(db, 'tasks', t.id))} className="text-red-200 hover:text-red-500"><Trash2 size={20}/></button>
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-white p-10 rounded-[3rem] border shadow-xl flex flex-col">
              <h3 className="text-xl font-black italic flex items-center gap-3 mb-8"><History className="text-stone-400" /> Storico</h3>
                ...
                {Array.from(new Set(tasks.filter(t => t.done).map(t => t.dateCompleted))).sort().reverse().map(date => (
                  <div key={date}>
                    ...
                    {tasks.filter(t => t.done && t.dateCompleted === date).map(i => (
                        <div key={i.id} className="flex justify-between items-center p-2"><p className="text-sm font-bold text-stone-400 line-through">• {i.text}</p><button onClick={() => deleteDoc(doc(db, 'tasks', i.id))} className="text-red-200 hover:text-red-500"><Trash2 size={16}/></button></div>
                    ))}
                  </div>
                ))}
              </div>
