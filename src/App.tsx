...
import {
  ...
  PlusCircle, LogOut, Lock, Wallet, ListChecks, Package,
  ArrowUpRight, ArrowDownLeft, Ghost, UserPlus, Search, Edit2, CheckCircle2, History, MinusCircle
} from 'lucide-react';
...
export default function App() {
  ...
  // Metodi di salvataggio omessi per brevità nella spiegazione (stessa logica del codice precedente, completi in questo blocco)
  const handleSaveAnimal = async () => {
    if (!newAnimal.name.trim()) return alert("Devi inserire un nome!");
  ...
  const handleSaveTransaction = async () => {
    if (!newTrans.desc.trim() || newTrans.amount <= 0) return alert("Inserisci dati validi!");
  ...
  const handleSaveBirth = async () => {
    if (!newBirth.idCode.trim() || newBirth.count < 1) return alert("Dati mancanti!");
    ...
    for (let i = 0; i < newBirth.count; i++) {
      await addDoc(collection(db, 'animals'), { name: `Figlio di ${newBirth.idCode} #${i + 1}`, species: newBirth.species, birthDate: newBirth.birthDate, dam: newBirth.idCode, notes: 'Parto', ownerId: user!.uid });
    }
  ...
  // DEFINIZIONE TABS PER BOTTOM NAV E SIDEBAR
  const tabs = [
  ...
  return (
    <div className="min-h-screen flex bg-stone-50 text-stone-900 pb-safe md:pb-0">
      ...
      <main className="flex-1 w-full md:ml-72 p-4 pt-24 md:pt-10 md:p-10">
        ...
        {/* KPI MINI-DASHBOARD (Ottimizzata per mobile) */}
        ...
        {/* --- SCHERMATE TABS (Design compatto per mobile) --- */}
        {activeTab === 'inventory' && (
          <div className="space-y-6">
            <div className="bg-white p-5 md:p-8 rounded-3xl border border-stone-100 shadow-sm">
              ...
              <div className="space-y-3 mb-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <select className="ui-input text-xs" value={newAnimal.sire} onChange={e => setNewAnimal({...newAnimal, sire: e.target.value})}><option value="">Padre</option>{validAnimals.filter(a => a.species === newAnimal.species).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
                  <select className="ui-input text-xs" value={newAnimal.dam} onChange={e => setNewAnimal({...newAnimal, dam: e.target.value})}><option value="">Madre</option>{validAnimals.filter(a => a.species === newAnimal.species).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
                </div>
                <textarea placeholder="Note sanitarie..." className="ui-input h-20 resize-none" value={newAnimal.notes} onChange={e => setNewAnimal({...newAnimal, notes: e.target.value})}></textarea>
              </div>
                    ...
                    {capi.map(a => (
                      <div key={a.id} className="bg-white p-5 rounded-2xl border border-stone-100 shadow-sm relative group flex flex-col">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <button onClick={() => { if(window.confirm("Eliminare?")) deleteDoc(doc(db, 'animals', a.id)); }} className="text-stone-300 hover:text-red-500 p-1"><Trash2 size={18}/></button>
                        </div>
                        ...
                        {(a.sire || a.dam) && (
                          <div className="bg-stone-50 p-2 rounded-lg text-[10px] font-bold text-stone-500 mb-2 flex gap-4">
                            {a.sire && <span>👨 {validAnimals.find(p=>p.id===a.sire)?.name}</span>}
                            {a.dam && <span>👩 {validAnimals.find(p=>p.id===a.dam)?.name}</span>}
                          </div>
                        )}
                        {a.notes && <p className="text-xs text-stone-500 italic mt-auto border-l-2 border-emerald-400 pl-2">{a.notes}</p>}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* ALTRE TABS (Design compresso) */}
        {activeTab === 'finance' && (
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-3xl border shadow-sm">
              <h3 className="text-xs font-black uppercase mb-4 text-emerald-900">Registra Entrata/Uscita</h3>
              <div className="space-y-3 mb-4">
                <input placeholder="Causale" className="ui-input" value={newTrans.desc} onChange={e => setNewTrans({...newTrans, desc: e.target.value})} />
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <select className="ui-input col-span-1 px-2" value={newTrans.type} onChange={e => setNewTrans({...newTrans, type: e.target.value as any})}><option>Entrata</option><option>Uscita</option></select>
                ...
                return (
                    <div key={s} className="bg-white p-5 rounded-2xl border shadow-sm">
                        <div className="flex justify-between items-center mb-4 border-b border-stone-100 pb-2">
                          <h4 className="text-lg font-black italic">{s}</h4>
                          <span className={`font-black ${balance >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>€ {balance.toFixed(2)}</span>
                        </div>
                        <div className="space-y-1">
                            {specTrans.sort((a,b)=>b.date.localeCompare(a.date)).map(t => (
                                <div key={t.id} className="flex justify-between items-center py-2 group">
                                    <div className="flex items-center gap-3">
                                      <div className={`p-1.5 rounded-lg ${t.type === 'Entrata' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      <button onClick={() => { if(window.confirm("Cancellare?")) deleteDoc(doc(db, 'transactions', t.id)); }} className="text-stone-300 hover:text-red-500"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
        ...
        {/* TABS RIMANENTI SINTETIZZATE MA COMPLETE LOGICAMENTE */}
        {activeTab === 'births' && (
          <div className="bg-white p-6 rounded-3xl border shadow-sm">
            <h3 className="text-xs font-black uppercase text-emerald-900 mb-5">Nuovo Parto</h3>
            <div className="space-y-3 mb-5">
              <input placeholder="Codice Madre" className="ui-input" value={newBirth.idCode} onChange={e => setNewBirth({...newBirth, idCode: e.target.value})} />
              <div className="grid grid-cols-2 gap-3">
                <input type="number" placeholder="Nati" className="ui-input" value={newBirth.count} onChange={e => setNewBirth({...newBirth, count: Number(e.target.value) || 1})} />
        ...
        {activeTab === 'products' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {products.map(p => (
                <div key={p.id} className="bg-white p-4 rounded-2xl border text-center shadow-sm">
                  ...
                  <div className="flex justify-center gap-4 border-t pt-3">
                    <button onClick={() => reduceProduct(p.id, 1)} className="text-stone-400 hover:text-red-500"><MinusCircle size={20}/></button>
                    <button onClick={() => deleteDoc(doc(db, 'products', p.id))} className="text-stone-400 hover:text-red-500"><Trash2 size={20}/></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-5 rounded-3xl border shadow-sm">
              <div className="flex gap-2 mb-5">
                <input className="ui-input flex-1 py-3" value={newTask} onChange={e => setNewTask(e.target.value)} placeholder="Da fare..." />
                <button onClick={async () => { if(newTask) { await addDoc(collection(db, 'tasks'), { text: newTask, done: false, ownerId: user.uid }); setNewTask(''); } }} className="bg-emerald-600 text-white px-4 rounded-xl font-black text-[10px] uppercase">Add</button>
              </div>
      ...
      <nav className="md:hidden fixed bottom-0 w-full bg-white/90 backdrop-blur-lg border-t border-stone-200 flex justify-between px-2 pb-safe z-50">
        {[
          ...
          <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center pt-3 pb-2 w-full transition-colors ${activeTab === item.id ? 'text-emerald-600' : 'text-stone-400'}`}>
