...
const DynastyBranch = ({ animal, allAnimals, level = 0 }: { animal: Animal, allAnimals: Animal[], level?: number }) => {
  ...
  return (
    <div className={level > 0 ? "ml-4 border-l border-stone-200 pl-4 mt-2" : ""}>
      <div className={`p-3 rounded-xl border bg-white mb-2 shadow-sm ${level === 0 ? 'border-l-4 border-l-emerald-500 font-bold text-emerald-900' : ''}`}>
        <p className="text-sm uppercase tracking-tighter">{animal.name}</p>
        <p className="text-[9px] text-stone-400 font-bold italic uppercase">{animal.species} • LIVELLO {level}</p>
      </div>
...
export default function App() {
  ...
  // --- LOGICA PUBBLICAZIONE MERCATO (CORRETTA) ---
  const handlePublishToMarket = async () => {
    if (!sellingProduct || sellPrice <= 0) return alert("Inserisci un prezzo valido.");
    
    try {
      // 1. Pubblica nel Mercato
      ...
      // 2. SOTTRAI DAL MAGAZZINO FISICO (Il bene ora è virtuale/in vendita)
      await deleteDoc(doc(db, 'products', sellingProduct.id));

      setSellingProduct(null);
      setSellPrice(0);
      setSellPhone('');
      alert("Prodotto rimosso dal magazzino e pubblicato con successo nel Mercato!");
      setActiveTab('market'); // Sposta l'utente al mercato per vedere il risultato
    } catch (e) {
  ...
  const exportASLReport = () => {
    const d = new jsPDF(); const n = window.prompt("Nome Azienda Agricola:") || "Azienda";
    d.text(n, 14, 20);
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-bold text-emerald-800 bg-stone-50 animate-pulse">Sincronizzazione Cloud...</div>;
      ...
      <div className="min-h-screen bg-stone-100 flex items-center justify-center p-4">
        <div className="bg-white p-6 rounded-3xl shadow-xl w-full max-w-sm border">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <button type="button" onClick={() => setIsRegistering(!isRegistering)} className="w-full text-xs font-bold text-stone-400 uppercase mt-4 text-center">{isRegistering ? "Login" : "Nuovo Utente? Registrati"}</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col md:flex-row relative text-stone-900 font-sans overflow-x-hidden">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-xl border-t flex justify-around p-2 z-50 shadow-lg overflow-x-auto hide-scrollbar">
        {menuItems.map(item => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center min-w-[65px] p-2 transition-colors ${activeTab === item.id ? 'text-emerald-600' : 'text-stone-400'}`}>
            <item.icon size={22} />
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-10 pb-24">
        <div className="mb-6 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 bg-white px-3 py-1 rounded-full border shadow-sm">
                ...
                <p className="text-[9px] font-bold text-stone-400 uppercase tracking-widest">{userName}</p>
            </div>
        </div>
        ...
        {activeTab === 'dashboard' && userRole === 'farmer' && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div onClick={()=>setActiveTab('finance')} className="bg-white p-5 rounded-3xl border shadow-sm cursor-pointer hover:border-emerald-500 transition-colors">
               <p className="text-[10px] font-bold text-stone-400 uppercase mb-1">Liquidità</p>
               <h4 className="text-2xl font-black italic">€{transactions.reduce((acc,t)=>acc+(t.type==='Entrata'?t.amount:-t.amount),0)}</h4>
            </div>
          </div>
        )}

        {/* --- 2. INVENTARIO (SPECIE + NOTE) --- */}
        {activeTab === 'inventory' && userRole === 'farmer' && (
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            <div className="bg-white p-5 rounded-2xl border shadow-sm">
              <div className="flex justify-between items-center mb-4 border-b pb-4">
                <h3 className="text-xs font-bold uppercase text-stone-400 italic tracking-widest">Registrazione Capi</h3>
                <button onClick={exportASLReport} className="text-[10px] font-bold bg-stone-900 text-white px-3 py-1 rounded-lg flex items-center gap-1 uppercase hover:bg-emerald-600 transition-all shadow-sm"><FileDown size={14}/> PDF ASL</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                <input placeholder="Codice / Tag ID" className="p-3 bg-stone-50 rounded-xl text-sm border-none shadow-inner uppercase font-bold" value={newAnimal.name} onChange={e=>setNewAnimal({...newAnimal, name:e.target.value})} />
                <select className="p-3 bg-stone-50 rounded-xl text-sm border-none shadow-inner uppercase font-bold" value={newAnimal.species} onChange={e=>setNewAnimal({...newAnimal, species:e.target.value as Species})}>{speciesList.map(s=><option key={s}>{s}</option>)}</select>
                <input type="date" className="p-3 bg-stone-50 rounded-xl text-sm border-none shadow-inner font-bold text-emerald-700" value={newAnimal.birthDate} onChange={e=>setNewAnimal({...newAnimal, birthDate:e.target.value})} />
                <input placeholder="Note (Salute, Trattamenti...)" className="p-3 bg-stone-50 rounded-xl text-sm border-none shadow-inner font-bold" value={newAnimal.notes} onChange={e=>setNewAnimal({...newAnimal, notes:e.target.value})} />
                <button onClick={handleSaveAnimal} className="bg-emerald-600 text-white font-bold rounded-xl p-3 uppercase text-xs shadow-lg hover:scale-105 transition-transform">Registra</button>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {capi.map(a => (
                      <div key={a.id} className="bg-white p-4 rounded-2xl border border-stone-100 shadow-sm relative group hover:border-emerald-500 transition-all">
                        <button onClick={()=>deleteDoc(doc(db,'animals',a.id))} className="absolute top-3 right-3 text-stone-200 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={16}/></button>
                        <h4 className="font-bold text-stone-800 uppercase text-sm leading-tight mb-1">{a.name}</h4>
                        <p className="text-[10px] text-stone-400 font-bold mb-3 italic tracking-tighter">{a.birthDate || 'N/D'}</p>
                        {a.notes && <p className="text-[10px] text-stone-500 border-t pt-2 mt-2 leading-relaxed italic">"{a.notes}"</p>}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {activeTab === 'births' && userRole === 'farmer' && (
          <div className="bg-white p-8 rounded-3xl border shadow-sm max-w-lg mx-auto animate-in zoom-in-95">
             <div className="bg-amber-50 p-6 rounded-2xl border border-amber-100 text-amber-700 mb-8 flex items-center gap-4 shadow-lg shadow-amber-50">
               ...
               <p className="text-[11px] font-bold italic uppercase tracking-tighter leading-relaxed text-amber-900">Le nascite verranno inserite automaticamente in anagrafica stalla collegandole alla madre.</p>
             </div>
             <div className="space-y-6">
                <input className="w-full p-4 bg-stone-50 rounded-xl font-black text-sm shadow-inner border-none uppercase" placeholder="Codice ID Madre" value={newBirth.idCode} onChange={e=>setNewBirth({...newBirth, idCode:e.target.value})} />
                <div className="flex gap-4">
                  <input type="number" className="flex-1 p-4 bg-stone-50 rounded-xl font-black text-sm shadow-inner border-none" placeholder="N. Nati" value={newBirth.count} onChange={e=>setNewBirth({...newBirth, count:Number(e.target.value)})} />
                  <select className="flex-1 p-4 bg-stone-50 rounded-xl font-black text-xs shadow-inner border-none uppercase tracking-widest" value={newBirth.species} onChange={e=>setNewBirth({...newBirth, species:e.target.value as Species})}>{speciesList.map(s=><option key={s}>{s}</option>)}</select>
                </div>
                <input type="date" className="w-full p-4 bg-stone-50 rounded-xl font-black text-sm shadow-inner border-none text-emerald-700" value={newBirth.birthDate} onChange={e=>setNewBirth({...newBirth, birthDate:e.target.value})} />
                <button onClick={handleSaveBirth} className="w-full bg-emerald-950 text-white py-5 rounded-2xl font-black uppercase text-xs shadow-2xl active:scale-95">Registra Parto</button>
             </div>
          </div>
        )}

        {/* --- 4. BILANCIO (FIX USCITE) --- */}
        {activeTab === 'finance' && userRole === 'farmer' && (
          <div className="space-y-6 animate-in slide-in-from-right-4">
            <div className="bg-stone-900 p-8 rounded-[2rem] text-white shadow-xl text-center relative overflow-hidden">
               <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mb-2 italic">Cash Flow Aziendale</p>
               <h3 className="text-5xl font-black italic tracking-tighter leading-none mb-4">€ {transactions.reduce((acc, t) => acc + (t.type === 'Entrata' ? t.amount : -t.amount), 0).toFixed(0)}</h3>
            </div>

            <div className="bg-white p-6 rounded-2xl border shadow-sm">
              <h3 className="text-[10px] font-black uppercase text-stone-400 mb-5 tracking-widest italic">Registra Movimento</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                <input placeholder="Causale" className="p-3 bg-stone-50 rounded-xl font-bold text-sm shadow-inner border-none uppercase lg:col-span-2" value={newTrans.desc} onChange={e=>setNewTrans({...newTrans, desc:e.target.value})} />
                ...
                {/* FIX: SELETTORE ENTRATA/USCITA */}
                <select className={`p-3 rounded-xl font-black text-[10px] border-none shadow-sm uppercase ${newTrans.type === 'Entrata' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`} value={newTrans.type} onChange={e=>setNewTrans({...newTrans, type:e.target.value as any})}>
                    <option value="Entrata">📈 Entrata</option>
                    <option value="Uscita">📉 Uscita</option>
                </select>
                <select className="p-3 bg-stone-50 rounded-xl font-bold text-[10px] shadow-inner border-none uppercase tracking-widest" value={newTrans.species} onChange={e=>setNewTrans({...newTrans, species:e.target.value as Species})}>{speciesList.map(s=><option key={s}>{s}</option>)}</select>
              </div>
              <button onClick={handleSaveTransaction} className="w-full mt-4 bg-emerald-600 text-white py-4 rounded-xl font-black uppercase text-xs shadow-lg active:scale-95 transition-all">Salva</button>
            </div>
            ...
            {speciesList.map(specie => {
              ...
              return (
                <div key={specie} className="space-y-2 animate-in fade-in">
                  <div className="flex justify-between items-center px-4 py-1.5 bg-stone-50 rounded-xl border-l-4 border-emerald-500 shadow-sm">
                    <h4 className="text-xs font-black text-stone-500 uppercase italic tracking-widest">{specie}</h4>
                    <span className={`text-[12px] font-black italic ${bilancioSpecie >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>Bilancio: €{bilancioSpecie}</span>
                  </div>
                  <div className="space-y-2">
                    {transSpecie.map(t => (
                      <div key={t.id} className="bg-white p-4 rounded-xl border border-stone-50 flex justify-between items-center shadow-sm">
                        <div><p className="font-bold text-xs text-stone-800 uppercase tracking-tighter leading-none mb-1">{t.desc}</p><p className="text-[9px] font-bold text-stone-300 uppercase tracking-widest italic">{t.date}</p></div>
                        <div className="flex items-center gap-4">
                            ...
                            <button onClick={()=>deleteDoc(doc(db,'transactions',t.id))} className="p-2 text-stone-100 hover:text-red-500 transition-colors"><Trash2 size={14}/></button>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {/* --- 5. SCORTE (TASTO VENDI KM 0) --- */}
        {activeTab === 'products' && userRole === 'farmer' && (
          <div className="space-y-8 animate-in fade-in">
            {/* MODALE DI PUBBLICAZIONE */}
            {sellingProduct && (
              <div className="bg-amber-50 p-6 rounded-3xl border-2 border-amber-400 shadow-2xl animate-in zoom-in-95 max-w-lg mx-auto mb-10 relative overflow-hidden">
                <div className="absolute top-0 right-0 bg-amber-400 text-white px-8 py-2 rounded-bl-2xl font-black text-[9px] uppercase tracking-widest shadow-md italic">Public Market</div>
                <h3 className="text-xl font-black text-amber-950 italic mb-6 uppercase flex items-center gap-4 leading-none"><ShoppingBag size={24}/> Vendi: {sellingProduct.name}</h3>
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-amber-600 ml-4 uppercase tracking-widest italic">Prezzo (€)</label>
                    <input type="number" className="w-full p-3 rounded-xl border-none shadow-md font-black text-lg text-emerald-600 bg-white" placeholder="0.00" onChange={e=>setSellPrice(Number(e.target.value))} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-amber-600 ml-4 uppercase tracking-widest italic">WhatsApp</label>
                    <input className="w-full p-3 rounded-xl border-none shadow-md font-black text-sm bg-white uppercase" placeholder="340..." onChange={e=>setSellPhone(e.target.value)} />
                  </div>
                </div>
                <div className="flex gap-4">
                  <button onClick={handlePublishToMarket} className="flex-1 bg-amber-500 text-white font-bold py-4 rounded-xl uppercase text-xs tracking-widest shadow-lg active:scale-95">Pubblica Ora</button>
            ...
            <div className="bg-white p-6 rounded-3xl border shadow-sm relative overflow-hidden border-t-4 border-emerald-600 shadow-sm">
              <h3 className="text-[10px] font-black uppercase text-stone-400 mb-6 tracking-widest flex items-center gap-2 italic"><Package className="text-emerald-500" size={14}/> Gestione Magazzino Fisico</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <input placeholder="Prodotto (es: Fieno)" className="p-3 bg-stone-50 rounded-xl font-bold text-sm border-none shadow-inner uppercase" value={newProduct.name} onChange={e=>setNewProduct({...newProduct, name:e.target.value})} />
                <div className="flex gap-2">
                ...
                <button onClick={handleAddProduct} className="bg-emerald-950 text-white rounded-xl font-bold uppercase tracking-widest shadow-lg hover:bg-emerald-600 transition-all text-xs active:scale-95">Aggiungi</button>
              </div>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {products.map(p => (
                <div key={p.id} className="bg-white p-5 rounded-3xl border border-stone-100 shadow-sm text-center flex flex-col group hover:border-emerald-200 transition-all duration-500">
                  ...
                  {/* QUESTO TASTO ORA APRE IL MODALE DI VENDITA */}
                  <button onClick={()=>setSellingProduct(p)} className="w-full bg-amber-100 text-amber-700 font-black py-3 rounded-xl text-[9px] uppercase tracking-widest mb-3 hover:bg-amber-500 hover:text-white transition-all flex items-center justify-center gap-2 border-2 border-amber-200 shadow-lg active:scale-95"><Store size={14}/> Vendi Km 0</button>
        ...
        {activeTab === 'tasks' && userRole === 'farmer' && (
          <div className="max-w-xl space-y-6 animate-in slide-in-from-left-6 mx-auto">
            <div className="bg-white p-6 rounded-3xl border shadow-sm space-y-5">
               <h3 className="text-[11px] font-black uppercase text-stone-400 mb-2 tracking-widest italic flex items-center gap-3 leading-none"><CalendarDays className="text-emerald-600" size={18}/> Pianificazione Lavori</h3>
               ...
               <div className="relative group">
                  ...
                  <input className="w-full p-3 pl-10 bg-stone-100 border-none rounded-xl font-bold text-xs uppercase italic shadow-inner shadow-stone-200 placeholder:text-stone-300" placeholder="Filtra lavori..." value={taskSearch} onChange={e=>setTaskSearch(e.target.value)} />
               </div>
            </div>
              ...
              {tasks.filter(t => t.text.toLowerCase().includes(taskSearch.toLowerCase()))
                ...
                <div key={t.id} className={`bg-white p-5 rounded-3xl border-2 flex justify-between items-center transition-all ${t.done ? 'opacity-30 grayscale' : 'shadow-sm border-white hover:border-emerald-500 group border-l-8 border-l-emerald-600'}`}>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={()=>deleteDoc(doc(db,'tasks',t.id))} className="p-3 bg-red-50 text-red-400 rounded-2xl hover:bg-red-600 hover:text-white transition-all shadow-xl"><Trash2 size={24}/></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'vet' && userRole === 'farmer' && (
          <div className="bg-white p-10 rounded-[3rem] border shadow-2xl max-w-2xl mx-auto animate-in zoom-in-95 mx-auto relative overflow-hidden">
            <div className="bg-blue-600 p-8 rounded-[2.5rem] text-white mb-10 flex items-center gap-6 shadow-2xl shadow-blue-200 group">
               ...
               <div className="relative z-10">
                  ...
                  <p className="text-blue-50 font-bold opacity-80 text-xs leading-relaxed italic tracking-tighter">Triage istantaneo clinico veterinario 2026.</p>
               </div>
            </div>
            ...
            <div className="space-y-6 mb-10">
                ...
                <div className="space-y-2">
                    ...
                    <textarea className="w-full p-8 bg-stone-50 border-none rounded-[3rem] font-bold text-stone-800 text-lg h-56 shadow-inner italic placeholder:text-stone-200 focus:ring-4 focus:ring-blue-100 transition-all leading-snug" placeholder="Descrivi il problema (es: vitello IT02 naso asciutto)..." value={vetSymptom} onChange={e=>setVetSymptom(e.target.value)}></textarea>
                </div>
            </div>

            <button onClick={()=>{setIsAnalyzing(true); setTimeout(()=>{setVetResult({title:"Esito Diagnostico IA", desc:"Il quadro clinico e visivo suggerisce una possibile infiammazione respiratoria acuta.", action:"ISOLARE IL CAPO E CONTATTARE IL VETERINARIO DI ZONA URGENTEMENTE"}); setIsAnalyzing(false);},3000)}} className="w-full bg-stone-950 text-white py-8 rounded-[2rem] font-black uppercase tracking-[0.4em] text-xs shadow-2xl active:scale-95">
              {isAnalyzing ? <><Activity className="animate-spin" size={28}/> Processing...</> : <><Bot size={36}/> Analizza</>}
            </button>
            ...
            {vetResult && (
              <div className="mt-16 p-10 bg-emerald-50 border-4 border-emerald-100 rounded-[3.5rem] animate-in slide-in-from-bottom-12">
                 ...
                 <p className="text-emerald-800 font-bold text-lg mb-10 italic leading-relaxed">"{vetResult.desc}"</p>
              ...
              {marketItems.map(item => (
                <div key={item.id} className="bg-white rounded-[3.5rem] border-2 border-stone-50 shadow-[0_30px_80px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col group hover:shadow-2xl hover:translate-y-[-10px] transition-all duration-700">
                  <div className="h-60 bg-stone-50 flex items-center justify-center relative overflow-hidden group-hover:bg-amber-50 transition-colors duration-1000 shadow-inner">
                     ...
                     <ShoppingBag size={80} className="text-stone-100 opacity-50 group-hover:scale-[1.8]" />
                  </div>
                  <div className="p-10 flex flex-col flex-1 relative">
                    ...
                    <div className="flex justify-between items-center mb-10 bg-stone-50 p-6 rounded-3xl border-2 border-white shadow-inner">
                       <span className="text-[11px] font-black text-stone-400 uppercase italic tracking-widest leading-none">In Stock</span>
