...
// Componente DynastyBranch con visualizzazione genitori
...
export default function App() {
  ...
  const handleSaveAnimal = async () => {
    ...
    // Validazione: se inserisci un padre, verifica che esista
    if (newAnimal.sire && !animals.some(a => a.name === newAnimal.sire || a.id === newAnimal.sire)) {
      if (!confirm(`Il padre "${newAnimal.sire}" non esiste nell'anagrafica. Continuare comunque?`)) {
        return;
      }
    }
    
    // Validazione: se inserisci una madre, verifica che esista
    if (newAnimal.dam && !animals.some(a => a.name === newAnimal.dam || a.id === newAnimal.dam)) {
  ...
  const exportASLReport = () => {
    ...
    autoTable(d, { 
      head: [['ID', 'Specie', 'Data Nascita', 'Padre', 'Madre', 'Note']], 
  ...
  const handleSaveBirth = async () => {
    ...
    try {
      ...
      for (let i = 0; i < newBirth.count; i++) {
        ...
        const customName = prompt(`Inserisci il nome per il capo n.${i + 1} (lascia vuoto per "${defaultName}"):`, defaultName) || defaultName;
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            {isRegistering && (
              <div className="space-y-3 bg-stone-50 p-4 rounded-2xl border">
                <input placeholder="Il tuo nome o azienda" className="w-full p-3 rounded-xl border text-sm" value={regName} onChange={e => setRegName(e.target.value)} required />
            ...
            <button type="submit" className="w-full bg-emerald-700 text-white py-3 rounded-xl font-bold uppercase shadow-lg active:scale-95">{isRegistering ? "Crea Account" : "Accedi"}</button>
            <button type="button" onClick={() => setIsRegistering(!isRegistering)} className="w-full text-xs font-bold text-stone-600 uppercase mt-4 text-center underline">{isRegistering ? "Hai già un account? Accedi" : "Non hai un account? Registrati"}</button>
          </form>
        </div>
      </div>
    );
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            <div className="bg-white p-4 rounded-2xl border shadow-sm space-y-3">
              <div className="flex justify-between items-center border-b pb-2">
                <h3 className="text-[10px] font-black text-stone-600 uppercase">Registrazione Nuovo Animale</h3>
                <button onClick={exportASLReport} className="text-[9px] font-bold bg-stone-900 text-white px-3 py-1 rounded-lg uppercase shadow-md">Esporta PDF</button>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <input 
                  placeholder="Nome/Codice animale *" 
                  className="p-3 bg-stone-50 rounded-lg text-xs font-bold border-none shadow-inner" 
                  value={newAnimal.name} 
                  onChange={e=>setNewAnimal({...newAnimal, name:e.target.value})} 
                />
                
                <select 
                  className="p-3 bg-stone-50 rounded-lg text-xs font-bold border-none shadow-inner uppercase" 
                ...
                <input 
                  type="date" 
                  className="p-3 bg-stone-50 rounded-lg text-xs font-bold border-none shadow-inner text-emerald-700" 
                  value={newAnimal.birthDate} 
                  onChange={e=>setNewAnimal({...newAnimal, birthDate:e.target.value})} 
                />
                
                <input 
                  placeholder="Padre (nome o codice)" 
                  className="p-3 bg-stone-50 rounded-lg text-xs font-bold border-none shadow-inner" 
                ...
                <input 
                  placeholder="Madre (nome o codice)" 
                  className="p-3 bg-stone-50 rounded-lg text-xs font-bold border-none shadow-inner" 
                  value={newAnimal.dam} 
                ...
                <input 
                  placeholder="Note (salute, cure, note)" 
                  className="p-3 bg-stone-50 rounded-lg text-xs font-bold border-none shadow-inner" 
                ...
                <button 
                  ...
                  className="bg-emerald-600 text-white font-bold rounded-lg py-3 text-xs uppercase col-span-full shadow-md active:scale-95"
                >
                  Salva Animale
                </button>
              </div>
              
              <p className="text-[8px] text-stone-400 italic mt-1">* campi obbligatori</p>
            </div>

            {/* Lista animali per specie */}
            {speciesList.map(specie => {
              ...
              return (
                <div key={specie} className="space-y-2">
                  <h4 className="text-xs font-black text-emerald-800 uppercase px-1 italic border-l-4 border-emerald-500 pl-2">
                    {specie} ({capi.length})
                  </h4>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {capi.map(a => {
                      // Trova i nomi dei genitori per visualizzarli
